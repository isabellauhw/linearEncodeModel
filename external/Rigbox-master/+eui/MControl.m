    ...
    function expSubjectChanged(obj, ~, src)
        ...
        switch src.EventName
            ...
            case 'Disconnected' % user logged out of Alyx
                obj.NewExpSubject.Option = unique([{'default'}; obj.getSubjectList]);
                obj.LogSubject.Option = unique([{'default'}; obj.getSubjectList]);
        end
    end
    
    function subjects = getSubjectList(obj)
        p = dat.paths;
        subjects = dat.listSubjects;
        
        %if MC hide list exists, then remove those subjects from the list
        mcHideListFile = fullfile(p.mainRepository,'.mc_hide_mice.txt');
        if exist(mcHideListFile,'file')
            ignoreSubjects = strsplit(fileread(mcHideListFile),',');
            subjects = setdiff(subjects,ignoreSubjects);
        end
    end
...
%           error('Experiment %s already running of %s', expDef, rig.Name)
    ...
    function buildUI(obj, parent) % Parent here is the MC window (figure)
      ...
      obj.LogSubject = bui.Selector(hbox, unique([{'default'}; obj.getSubjectList])); % Subject dropdown box, Child of hbox
      ...
      obj.NewExpSubject = bui.Selector(topgrid, unique([{'default'}; obj.getSubjectList])); % Subject dropdown box
