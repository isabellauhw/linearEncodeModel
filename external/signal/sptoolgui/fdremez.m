...
case 'init'
    ...
    if ~strcmp(bandpop.userdata,'fdremez')
       % this module is not current
        bandpop.userdata = 'fdremez';
            
        minOrdCheckbox.callback = 'fdremez(''checkbox'')';
        bandpop.callback = 'fdremez(''newtype'')';         
        set(pbspecs(1),'callback','fdremez(''fchange'')','range',[0 Fs/2])
        set(pbspecs(2),'callback','fdremez(''fchange'')','range',[0 Fs/2])
        set(sbspecs(1),'callback','fdremez(''fchange'')','range',[0 Fs/2])
        set(sbspecs(2),'callback','fdremez(''fchange'')','range',[0 Fs/2])
        ...
        Rpm = pbmeasures(1);
        set(Rpm,'label',[getString(message('signal:sptoolgui:Actual')) ' Rp'],'format','%1.4g','visible','on')
        Rsm = sbmeasures(1);
        set(Rsm,'label',[getString(message('signal:sptoolgui:Actual')) ' Rs'],'format','%1.4g','visible','on')
        wt1m = pbmeasures(2);
        wt2m = sbmeasures(2);
        set(wt1m,'label',getString(message('signal:sptoolgui:Weight')),'format','%1.4g')
        set(wt2m,'label',getString(message('signal:sptoolgui:Weight')),'format','%1.4g')
        
        fdutil('changeToText',[pbmeasures(1:2); sbmeasures(1:2)])

        set(ax,'title',getString(message('signal:sptoolgui:FrequencyResponse')),'xlimbound',[0 Fs/2],...
              'xlim',[0 Fs/2],...
        ...
        set(Lresp,'buttondownfcn','fdremez(''LrespDown'')')
                       
        set(L1,'buttondownfcn','fdremez(''L1down'')',...
               'buttonupfcn','fdremez(''L1up'')');
    
        set(L2,'buttondownfcn','fdremez(''L2down'')',...
               'buttonupfcn','fdremez(''L2up'')');
    ...
    fdremez('newfilt',setOrderFlag_init,type_init,f_init,Rp_init,...
...
case 'apply'
    ...
    if ~setOrderFlag     % estimate order
        [n,f,m,wt] = estimateOrder(type,Rp.value,Rs.value,Fs,...
                 f1.value,f2.value,f3.value,f4.value);
        order.value = n;
        wt1m.value = wt(find(m(1:2:end)==1, 1 ));
        wt2m.value = wt(find(m(1:2:end)==0, 1 ));
    else
        n = order.value;
        switch type
        case 1   % lowpass
    ...
    if ((type==2)|(type==4)) & rem(n,2)
        error(message('signal:fdremez:MustBeEven'))
    end
    b = remez(order.value,f,m,wt);
    % Trap NaNs and Infs because creating a filter structure with NaNs or Infs will
    % fail the test "isequal" when comparing fiter structures.
    b(find(isnan(b) | isinf(b)))=eps; 
    a = 1;
    ...
    specstruc.setOrderFlag = ~minOrdCheckbox.value;
    ...
    if ~specstruc.setOrderFlag
    % set ydata of L1 and L2 according to desired values of Rp, Rs,
    %  not actual values
...
case 'revert'
    ...
    setOrderFlag = filt.specs.fdremez.setOrderFlag;
    f = filt.specs.fdremez.f;
    Rpass = filt.specs.fdremez.Rp;
    Rstop = filt.specs.fdremez.Rs;
    wt = filt.specs.fdremez.wt;
    N = filt.specs.fdremez.order;
    fdremez('newfilt',setOrderFlag,oldtype,f,Rpass,Rstop,wt,N)
...
case 'help'
    str = fdhelpstr('fdremez');
    varargout{1} = str{2};

case 'description'
    varargout{1} = getString(message('signal:sptoolgui:EquirippleFIR'));
...
case 'newfilt'
    ...
    if ~setOrderFlag
        ...
        set(wt1,'value',wt(1+(type == 2 | type == 3)),...
           'label',getString(message('signal:sptoolgui:Weight')),'callback','fdremez(''dirty'')')
...
% LocalWords:  Krauss newtype fchange Rp xlimbound Lresp newfilt Ns fiter nfft
% LocalWords:  xdata ydata Fs passband xlimpassband Rpchange maxpass minpass
% LocalWords:  Rschange bandpop passframe userdata Fp getenable Lrespdown
% LocalWords:  stopband Rpdrag Rsdrag fdcheby IIR
