...
switch action
    ...
    case 'setfilt'
        ...
        if ~isstruct(f.specs) | ~isfield(f.specs,'currentModule') | ...
                isempty(find(strcmp(ud.modules,f.specs.currentModule), 1))
            switch f.type
                case 'design'
    ...
    case 'SPTclose'
        ...
        if(isequal(filtdesPrefs.fdatoolFlag, 1))
            ...
            if ~isempty(fig)
                ...
                for i=1:length(ud.modules)
                    if ~isempty(find(strcmp(fcnList,ud.modules{i}), 1))
...
switch action
  case {'create','change'} % "New" or "Edit" button is clicked
    if isfield(ud, 'hFDA') && ~isempty(ud.hFDA) && ishandle(ud.hFDA)
      hFDA = ud.hFDA;
    else
      hFDA = filterDesigner(opts);
    end
end

if ~isempty(hFDA)
  % If the FDAtool is not rendered, the handle will be empty. In this case
  % the following section is skipped.
