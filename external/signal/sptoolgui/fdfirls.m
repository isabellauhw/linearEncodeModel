...
case 'apply'
    ...
    if ((type==2)|(type==4)) & rem(n,2)
        error(message('signal:fdfirls:MustBeEven'))
    end
...
case 'description'
    varargout{1} = getString(message('signal:sptoolgui:LeastSquaresFIR'));
...
%     
%     error('signal:fdfirls:GUIErr','For FIRLS, you can''t select "Minimum Order".')
...
case 'newfilt'
    ...
    set(wt1,'value',wt(1+(type == 2 | type == 3)),...
       'label',getString(message('signal:sptoolgui:Weight')),'callback','fdfirls(''dirty'')')
...
% LocalWords:  newtype fchange Rp xlimbound Lresp newfilt nfft xdata ydata Fs
% LocalWords:  Ns passband xlimpassband Rpchange maxpass minpass Rschange Ord
% LocalWords:  bandpop passframe userdata Fp getenable Lrespdown stopband
% LocalWords:  Rpdrag Rsdrag fdcheby IIR
