...
case 'init'
    ...
    ud.list = uicontrol('Style','listbox',...
      'Units','pixels',...
      'String',{prefs([1:4 6:end]).panelDescription},...
      'Value',panelInd2listInd(panelInd),...
      'Tag','listbox',...
...
%     designer
    ...
    while ~done
        ...
        case 'list'
            ...
            panelInd = listInd2PanelInd(get(ud.list,'Value'));
...
% These helper functions are used to remove Filter Designer from the
% preference menu. They convert between the panelInd (which includes Filter
% Designer) and UI list indicies (which does not).
function panelInd = listInd2PanelInd(listInd)
    if listInd >= 5
      panelInd = listInd+1;
    else
      panelInd = listInd;
...
function listInd = panelInd2listInd(panelInd)
    if panelInd >= 5
      listInd = panelInd-1;
    else
      listInd = panelInd; 
