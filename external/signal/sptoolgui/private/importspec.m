...
%    'fields' - return information about the input fields and field names
%    for
...
case 'fields'
  ...
  popupString = getString(message('signal:sptoolgui:Spectrum'));
  flds.form = 'Spectrum';
  flds.fields = {'PSD' getString(message('signal:sptoolgui:FreqVector'))};
  FsFlag = 0;
  defaultLabel = 'spect';
...
%  since the fields and their order have to be the same for every object!
    ...
    if isempty(errstr)
        if ~ismatrix(struc.P) || ~isa(struc.P,'double')
           errstr = getString(message('signal:sptoolgui:TheDataForTheSpectrumMustBeA2DMatrixOfClassdouble'));
        end
    ...
    if isempty(errstr)
        if ~ismatrix(struc.f) || ~isa(struc.f,'double')
           errstr = getString(message('signal:sptoolgui:MustBeA2DMatrixOfClassdouble'));
        end
    ...
    if isempty(errstr)
        if ~isequal(length(struc.P),length(struc.f))
            errstr = getString(message('signal:sptoolgui:TheLengthsOfPSDAndFrequencyVectorsMustBeTheSame'));
        end
    end
    if isempty(errstr)
        if ~isreal(struc.f)
            errstr = getString(message('signal:sptoolgui:TheFrequencyVectorMustBeReal'));
        end
    end
    if isempty(errstr)
        if ~isreal(struc.P) || any(struc.P<0)
            errstr = getString(message('signal:sptoolgui:TheInputSpectralDataMustBeRealAndNonnegative'));
        end
    end
...
%     the signals in SPTOOL are checked for the existence of this signal.
...
%     flag of the found signal, and the P and f fields are cleared.
    ...
    [err,errstr,vanillaStruc]=importspec('make',{1 1 1}); %#ok<*ASGLU>
    ...
    % Update the specs field , if it exists;  When importing a spectra, the
    % specs information is not always known (see the comments on lines 495
    % - 499 of the spectview.m file).
    if valid && ~isempty(struc.specs)
...
struc = varargin{1};
struc.type = 'auto';
% As part of g230129, tried to update the appended string for the case
% when importing a spectra. The problem is that this would always add the
% 'imported' string because the specs fields is initially empty.
