...
if strcmpi(msg,'pzedit')
    ...
    %  function which returns the indices of the filters to be updated.
...
% Obtain the indices ('Indx') of the filter objects to be created
if strcmpi(msg,'pzedit')
...
% fdatool - use existing filter object

k = 1;
% Create the dfiltwfs objects
while k <= kLim
    if isfield(filtStruc(Indx(k)), 'FDAspecs') && ~isempty(filtStruc(Indx(k)).FDAspecs) && isfield(filtStruc(Indx(k)).FDAspecs, 'current_filt')
        dFilt = filtStruc(Indx(k)).FDAspecs.current_filt;        
    else
        dFilt = dfilt.df1(filtStruc(Indx(k)).tf.num,...
            filtStruc(Indx(k)).tf.den);
    end
    filtObj = [filtObj dfilt.dfiltwfs(dFilt,...
            filtStruc(Indx(k)).Fs,...
...
% If yes set the boolUpdate flag to 1 & also obtain their indices.
switch msg
    case {'new'}                
        % Multiple filters from SPTool
        for k = 1 : length(filtObj)
            fsFlag = ~isequal(filtObj(k).Fs,filtStruc(k).Fs);
            nameFlag = ~isequal(filtObj(k).Name,filtStruc(k).label);
            
            % Since filtObj(k).Filter can now (R14SP4) contain strucutres
            % other than DF1, we force an update when the DFILT was created
            % in FDATool (numFlag & denFlag = 1), and check for other
            % cases. This avoids creating a switch case for all other
            % structures
            numFlag = 1;
            denFlag = 1;
            if isa(filtObj(k).Filter,'dfilt.df1')
                numFlag = ...
