...
case 'init'
    ...
    if ~strcmp(bandpop.userdata,'fdkaiser')
       % this module is not current
        ...
        set(ax,'title',getString(message('signal:sptoolgui:FrequencyResponse')),...
...
%     end
    ...
    if ((type==2)|(type==4)) & rem(n,2)
        error(message('signal:fdkaiser:MustBeEven'))
    end
...
% LocalWords:  Krauss newtype xlimbound Lresp xdata segmentdragcallback newfilt
% LocalWords:  noscale nfft Fs ydata linestyle setenable fchange Ord Ns
% LocalWords:  passband xlimpassband Rpchange Rp maxpass minpass Rschange Wn
% LocalWords:  bandpop passframe userdata Fc segmentdragmode segmentpointer
% LocalWords:  vertexdragmode vertexpointer getenable Lrespdown buttondown
% LocalWords:  stopband Rpdrag Rsdrag Hlog lp bp bs minstop fm rmin rmax
% LocalWords:  RChange zeropad Fp dragmodes fdcheby IIR
