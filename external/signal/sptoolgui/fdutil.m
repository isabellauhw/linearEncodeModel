...
if ~isempty(mocb_ind)
    ...
    bandpop = fdspec('style','popupmenu',...
             ...
             'string',{getString(message('signal:sptoolgui:Lowpass')) getString(message('signal:sptoolgui:Highpass')) getString(message('signal:sptoolgui:Bandpass')) getString(message('signal:sptoolgui:Bandstop'))},...
             'label','Type',...
    ...
    ax = fdax('tag','ax','title',getString(message('signal:sptoolgui:FrequencyResponse')),...
              'xlabel',getString(message('signal:sptoolgui:Frequency')),...
              'ylabel',getString(message('signal:sptoolgui:MagnitudedB')),...
              'help','fdobjhelp');
    ...
    Lresp = fdline('tag','response','color',co(1,:),...
                       'help','fdobjhelp');
...
%    and prompt user if they really want to do this
    str = { [num2str(n) ' ' getString(message('signal:sptoolgui:IsAVeryLarge'))]
             getString(message('signal:sptoolgui:AreYouSure'))};
    if any(strcmp(msg,{'motion', 'up'}))
        continueFlag = 0;
    else
        ans = questdlg(str,getString(message('signal:sptoolgui:LargeFilterOrder')),'Yes','No','No');
        continueFlag = strcmp(ans,'Yes');
    end
    str = getString(message('signal:sptoolgui:FilterOrderTooLargeFor',num2str(n)));
...
%    f - frequencies = linspace(fstart,fend,M)
    ...
    c = {M exp(-1i*delta_f)  exp(1i*fstart*2*pi/Fs)};
...
% Outputs:
%    H - frequency response
    B = exp(-1i*(f(:)*2*pi/Fs)*(0:length(num)-1))*num(:);
    A = exp(-1i*(f(:)*2*pi/Fs)*(0:length(den)-1))*den(:);
    H = B./A;
...
%             of why the str is bad.
    ...
    if all(size(val)==1)
        if ~complex & val~=real(val)
            validVal = 0;
            errstr = getString(message('signal:sptoolgui:SorryThisValueMustBeReal'));
        end
        if validVal & (integer & val~=round(val))
            validVal = 0;
            errstr = getString(message('signal:sptoolgui:SorryThisValueMustBeAnInteger'));
        end
        ...
        validVal = 0;
        errstr = getString(message('signal:sptoolgui:SorryTheValue'));
    end
    if ~isempty(errstr)
        val = [];
    end
...
%rangeErrStr
    ...
    if all(inclusive == [1 1])
        errstr = sprintf(getString(message('signal:sptoolgui:SorryThisValueMustBeAtLeastgAndAtMostg')),...
                                  range(1),range(2));
    elseif all(inclusive == [1 0])
        errstr = sprintf(getString(message('signal:sptoolgui:SorryThisValueMustBeAtLeastgAndLessThang')),...
                                  range(1),range(2));
    elseif all(inclusive == [0 1])
        errstr = sprintf(getString(message('signal:sptoolgui:SorryThisValueMustBeGreaterThangAndAtMostg')),...
                                  range(1),range(2));
    elseif all(inclusive == [0 0])
        errstr = sprintf(getString(message('signal:sptoolgui:SorryThisValueMustBeGreaterThangAndLessThang')),...
                                  range(1),range(2));
    else
        error(message('signal:fdutil:GUIErr'))
    end
...
% describe the filter specifications
    ...
    if isempty(cfig)
    % create figure
        cfig = figure('Name',getString(message('signal:sptoolgui:BandConfigurations')),...
            'DockControls','off',...
  ...
  -4.2476e-06
    ...
    title(getString(message('signal:sptoolgui:ULowpass')))
    grid on
    ...
    t1=text(0,0,'Rp \{ ');
    set(t1,'HorizontalAlignment','right','Clipping','off')
    title(getString(message('signal:sptoolgui:UHighpass')))
    grid on
    ...
    t1=text(0,0,'Rp \{ ');
    set(t1,'HorizontalAlignment','right','Clipping','off')
    title(getString(message('signal:sptoolgui:UBandpass')))
    grid on
    ...
    t1=text(0,0,'Rp \{ ');
    set(t1,'HorizontalAlignment','right','Clipping','off')
    title(getString(message('signal:sptoolgui:UBandstop')))
    grid on
...
% LocalWords:  Krauss newtype lp bp bs oldtype splitwidth xd dxd hlabel
% LocalWords:  newlabel userdata fdhelpstr vect uicontrol's fdspecs
% LocalWords:  togglebutton popupmenu fdcheby TPK minordcheckbox bandpop
% LocalWords:  passframe stopframe pb sb pbm sbm passband stopband fdobjhelp
% LocalWords:  linewidth xdata ydata affectlimits segmentdragmode lr
% LocalWords:  segmentpointer lrdrag backgroundcolor xlimits maxima
% LocalWords:  unnormalized Fs lastvalue df fstart dft errstr IIR
% LocalWords:  vertexdragmode vertexdragcallback vertexpointer ud
% LocalWords:  segmentdragcallback Rpdrag uddrag Rsdrag newfilt RMAX RMIN Fp
% LocalWords:  Fsamp YTick Rp YLim
