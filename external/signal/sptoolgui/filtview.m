...
if nargin == 0
    if isempty(findobj(0,'tag','sptool'))
        ...
        disp(getString(message('signal:sptoolgui:ToUseTheFilterViewer')))
    end
    return
end
action = varargin{1};
...
for k = 1 : length(filtStruc)
    if isfield(filtStruc(k), 'FDAspecs') && ~isempty(filtStruc(k).FDAspecs) && isfield(filtStruc(k).FDAspecs, 'current_filt')
        % Use the Filter object created in FDATool (regardless of
        % structure)
        DFILT = filtStruc(k).FDAspecs.current_filt;
    else
        % Legacy code which always converts to DF1.
        DFILT = dfilt.df1(filtStruc(k).tf.num,filtStruc(k).tf.den);
    end
    DFILTWFS(k) = dfilt.dfiltwfs(DFILT);
...
hFVT = getcomponent(hFig,'siggui.fvtool');
...
closeList = addlistener(hFVT, 'sigguiClosing',@(s,e)closing_listener(s,e,SPToolFig,k));
  
sigsetappdata(hFVT.FigureHandle, 'sptool', 'closelistener', closeList);

% Setup a listener on FVTool NewPlot Event
newPlotListener = addlistener(hFVT, 'NewPlot',@(s,e)newplot_listener(s,e,SPToolFig));

sigsetappdata(hFVT.FigureHandle, 'sptool', 'newplotlistener', newPlotListener);
...
% LocalWords:  FV Malladi Tfig Fs dup userdata Tclose Aspecs DF FVT
% LocalWords:  INSTALLLISTENERS closelistener newplotlistener LCLSELECTION btn
% LocalWords:  SPTOOLSTRUC filtercoeffs magnphaseresp impresp stepresp
% LocalWords:  unitcircle freqmode timemode REMOVEREPLICAS nfft
