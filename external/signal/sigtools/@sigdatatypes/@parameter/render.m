...
narginchk(3,6);
[specPos, format, autoupdate] = parse_inputs(pos, varargin{:});
...
if isempty(specPos)
    hPrmName = getTranslatedStringcell('signal:sigtools:sigdatatypes', get(hPrm, 'Name'));
    lblwidth = largestuiwidth(cellstr(hPrmName))+5*pixf;
    
    strings = get(hPrm, 'ValidValues');
    vv = getTranslatedStringcell('signal:sigtools:sigdatatypes',  strings);
...
name = get(hPrm, 'Name');
name = getTranslatedStringcell('signal:sigtools:sigdatatypes',  name);
...
if iscell(hPrm(1).ValidValues)
    ...
    strs = getTranslatedStringcell('signal:sigtools:sigdatatypes',strs);
else
    strs = ' ';
end
...
if iscell(name)
    ...
    popPos(3) = check_position(popPos, specPos) - 10;
...
set(hpopup, 'Value', indx, 'String', getTranslatedStringcell('signal:sigtools:sigdatatypes',vv));
...
if ~isfield(h,'popup')
    ...
    poptag = get(h.popup,'Tag');
    if strcmpi(get(hPrm,'Tag'), poptag(1:strfind(poptag,'_')-1))
        sync_specifier(hPrm, h);
    end
end
...
strs = get(hPrm, 'ValidValues');
strs = getTranslatedStringcell('signal:sigtools:sigdatatypes',strs);
set(h.specpopup, 'String', strs);
...
if iscell(vv)
    ...
    vvt = getTranslatedStringcell('signal:sigtools:sigdatatypes',vv);
    if isempty(indx)
        value = popupstr(h.specpopup);
        indx = find(strcmpi(value, vvt));
        if isempty(indx), indx = get(h.specpopup, 'Value'); end
    end
    set(h.specpopup, 'String', vvt, 'Value', indx);
elseif ischar(vv) & strcmpi(vv, 'on/off')
...
if iscell(hActive.ValidValues)
    ...
    set(h.specpopup, 'String', getTranslatedStringcell('signal:sigtools:sigdatatypes',hActive.ValidValues), ...
        'Visible', 'On');
...
try,
    ...
    errordlg(cleanerrormsg(ME.message), getString(message('signal:sigtools:sigdatatypes:ParameterError')));
...
switch lower(get(hcbo, 'Style'))
    ...
    otherwise
        valueIdx = get(hcbo,'Value');
        validValuesStr = hPrm.ValidValues;
        value = validValuesStr{valueIdx};
end
...
% ----------------------------------------------------------------------
function [specPos, format, autoupdate] = parse_inputs(pos, varargin)
...
for i = 1:length(varargin)
    ...
    elseif isnumeric(varargin{i})
        ...
        case 0
          ...
          otherwise
            error(message('signal:sigdatatypes:parameter:render:SigErr'))
        end
    end
end
