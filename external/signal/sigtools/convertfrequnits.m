...
% Parse the inputs
[Frequency, Fs, Previous, Current, All] = parse_inputs(varargin{:});
...
% normalized units
if isempty(Fs)
    error(message('signal:convertfrequnits:InvalidFreqVec'))
end
...
if ~isa(inputFrequency,'cell') && ~isa(inputFrequency,'double') && ~isa(inputFrequency,'char')
    error(message('signal:convertfrequnits:InvalidParam'))
end
...
case 'cell'
    for n = length(inputFrequency):-1:1
...
elseif isa(Opts,'double')
    ...
    error(message('signal:convertfrequnits:MustBeAString'))
end

if isa(Frequency,'double')
    ConvertedFs = Frequency.*power(10,dist*3);
elseif isa(Frequency,'char')    
    [strFrequency, errMssg, ~] = evaluatevars(Frequency);
    if isempty(errMssg)
        ...
        if ~isempty(strFrequency) && ~isnan(strFrequency)
            ...
            ConvertedFs = [Frequency '*' num2str(power(10,dist*3))];
        end
    end
elseif isa(Frequency,'cell')
    % Place holder in case of cell array
    for i = length(Frequency):-1:1
...
%-------------------------------------------------------------------------------
function [freq,fs,pu,cu,au] = parse_inputs(varargin)
...
if strncmpi(pu,'norm',4) || strncmpi(cu,'norm',4)
    if isempty(fs)
        error(message('signal:convertfrequnits:SigErr', 'FS'));
    end
end
