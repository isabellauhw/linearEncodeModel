...
if isempty(sys) || ~isstr(sys)
  error(message('signal:dspfwiztargets:realizemdltarget:interstg:MustBeAString'));
end
if ~isequal(size(S1,2),size(S2,2)) || ~iscell(S1) || ~iscell(S2)
  error(message('signal:dspfwiztargets:realizemdltarget:interstg:InvalidDimensions'));
end
...
if nargin<4
  ...
  if length(order)~=num_conns
    error(message('signal:dspfwiztargets:realizemdltarget:interstg:InvalidDimensionsOrder', 'ORDER'));
  end
  i = find(order==12 | order==21);
  if ~isequal(length(i),length(order))
    error(message('signal:dspfwiztargets:realizemdltarget:interstg:GUIErrDirection', 'ORDER'));
  end
end
      ...
      for j = 1:length(sink_list) % Loop over all sinks for this connection
        ...
        elseif isnumeric(curr_sink)
          ...
          if any(order(i) == order(curr_sink))
            error(message('signal:dspfwiztargets:realizemdltarget:interstg:GUIErrShort'));
          end
          ...
          if length(curr_sink)>1
            error(message('signal:dspfwiztargets:realizemdltarget:interstg:NotSupported'));
          end
          ...
          for k = 1:length(short_list)  % Loop over sinks to short to
           ...
           if isnumeric(Ssrc{1,i})
              error(message('signal:dspfwiztargets:realizemdltarget:interstg:InternalErrorShortCircuit'));
           end
