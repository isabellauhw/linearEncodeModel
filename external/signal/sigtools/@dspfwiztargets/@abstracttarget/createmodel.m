...
if strcmpi(targetsubsys,'current')
    if islibrary(gcs) || syslinked(gcs)
...
elseif strcmpi(targetsubsys,'new')
    ...
    sys = uniq_sys;
    new_system(sys, 'Model');
elseif ~isempty(hTar.system) && ~isempty(find_system(hTar.system))
    % If current system is open
    sys = hTar.system;
elseif isempty(targetsubsys)
    error(message('signal:dspfwiztargets:abstracttarget:createmodel:InvalidSubSystemEmpty'));
elseif ~sysexist(targetsubsys)
    error(message('signal:dspfwiztargets:abstracttarget:createmodel:InvalidSubSystemNotExist', targetsubsys));
else
    % User Defined
    sys = targetsubsys;
end
...
if isempty(slindex) % will be empty only when sysname is Filter
    ...
    if isempty(currentblk)
             warning(message('signal:dspfwiztargets:abstracttarget:createmodel:FailedOverwrite', sysname)); % overwrite block which does not exists
    else
...
if send_warn
    slindex = strfind(new_sys,'/');
    if ~isempty(slindex)
        blockname = new_sys(slindex(end)+1:end);
        warning(message('signal:createmodel:NameChanged', blockname));
    end
end
...
%-----------------------------------------------------------
function y = islibrary(sys)
%ISLIBRARY Determine whether a system is a library.

y = ~isempty(sys);
if y
   y = strcmpi(get_param(bdroot(sys), 'BlockDiagramType'),'library');
end
