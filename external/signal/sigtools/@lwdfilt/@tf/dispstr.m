...
[num, den] = lcldispstr(this.Numerator(:), this.Denominator(:), varargin{:});

s = char({getString(message('signal:sigtools:siggui:Numerator'))
          num
          getString(message('signal:sigtools:siggui:Denominator'))
          den
         });
...
switch lower(fmt(1:3))
    ...
    case 'bin'
        if isfixptinstalled
            ...
            error(message('signal:lwdfilt:tf:dispstr:invalidFormat'));
        end
end
