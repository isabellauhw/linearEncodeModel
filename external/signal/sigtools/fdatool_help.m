...
% --------------------------------------------------------------
function toolhelp_cb(hco,eventStruct) %#ok<*INUSD>
% toolhelp_cb Overview of the tool (reference-page help).

helpview(fullfile(docroot,'toolbox','signal','signal.map'), 'fdatool_overview');
...
% --------------------------------------------------------------
function [tag, tbx] = designspecs_tags(hFig,frame)
...
[~, method] = strtok(get(hd, 'DesignMethod'), '.');
method(1) = [];
filterType = get(hd, 'SubType');

tag = ['fdatool_',filterType,'_',frame,'_specs_',method];

if any(strcmpi(method, {'firlpnorm', 'iirlpnorm', 'iirlpnormc', 'gremez', ...
        'fircband', 'ifir', 'firceqrip', 'iircomb', 'iirpeak', 'iirnotchpeak'})) || ...
        strcmpi(filterType, 'halfbandhp')
    tbx = [filesep 'dsp'];
end

% --------------------------------------------------------------
function [tag, tbx] = freqmagspecs_tags(hFig)
...
if any(strcmpi(filterType, {'arbitrarygrp'}))
    tbx = [filesep 'dsp'];
end
