...
if rem(N,2)
    ...
    error(message('signal:fmethod:abstractclassiciir:designparameq:oddOrder'));
end
...
switch type
  ...
  case 3
    ...
    k = ellipdeg(N, k1, tol);
    if G0~=0 
       ju0 = asne(1i*G/e/G0, k1, tol)/N;    % not used when G0=0
    end    
    jv0 = asne(1i/e, k1, tol)/N;
end
  ...
  switch type
    ...
    case 3
      ...
      elseif G0~=0 && G==0
         ...
         z0 = real(1i*cde(-1+ju0,k,tol));    % it's supposed to be real
         B00 = G*WB; B01 = -G/z0;
      end    
      p0 = real(1i*cde(-1+jv0,k,tol));
      A00 = WB; A01 = -1/p0;
      Ba(1,:) = [B00,B01,0];
      Aa(1,:) = [A00,A01,0];
   end
...
if L>0 
   ...
   switch type
      ...
      case 3
        if G0==0 && G~=0
           zeros = 1i ./ (k*cde(ui,k,tol));
        elseif G0~=0 && G==0
           zeros = 1i*cde(ui,k,tol);
        else                               % G0~=0 and G~=0
           zeros = 1i*cde(ui-ju0,k,tol);
        end
        poles = 1i*cde(ui-jv0,k,tol);
