function [s,g] = bilineardesign(h,has,c)
%BILINEARDESIGN  Design digital filter from analog specs. using bilinear. 

%   Copyright 1999-2017 The MathWorks, Inc.

N = has.FilterOrder;
if rem(N,2) == 1
    error(message('signal:fmethod:cheby1bs:bilineardesign:oddOrder'));
end
if rem(N,4) == 0 && any(strcmpi(h.FilterStructure,{'cascadeallpass','cascadewdfallpass'}))
    error(message('signal:fmethod:cheby1bs:bilineardesign:twiceEvenOrder'));
end

wp = has.Wpass;
rp = has.Apass;

% Compute Chebyshev I coeffs
[num,a1,w0] = cheby1coeffs(h,N/2,wp,rp);

% Compute den coeffs
den = 1+a1+num;
ai1 = (-2*c*a1-4*c*num)./den;
ai2 = 2*(2*c^2*num+num-1)./den;
ai3 = 2*c*(a1-2*num)./den;
ai4 = (1+num-a1)./den;

fog = (num)./den; %Fourth-order gains

% Initialize matrix
[s,g] = sosinitbpbs(h,N,ai1,ai2,ai3,ai4,fog);

% Set all numerators
msf = 2*floor(N/4);
s(1:msf,1:3) = repmat([1 -2*c 1],msf,1);

if rem(N,4)
    s(end,1:3) = [1 -2*c 1];
    s(end,4:6) = [1 -2*c*w0/(w0 + 1) (w0-1)/(w0 + 1)];
    g(end) = w0/(w0+1);
else
    g(end+1) = sqrt(10^(-rp/10));
end


% [EOF]
