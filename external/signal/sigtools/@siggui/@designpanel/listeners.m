...
mode = getmode(this);
if ~isa(hDM,'filtdes.gremez') && ...
        any(strcmp(mode,{'minimum odd','minimum even'}))
    mode = 'minimum';
end
...
if ~isempty(iir)
    ...
    allstrs = getTranslatedStringcell('signal:sigtools:siggui', allstrs);
...
for i = 1:length(hfound)
    if isempty(hfound{i})
        ...
        elseif this.IsLoading
            ...
            % when closing an fdatool which was opened from dspblk
            % isLoading is true but we should not ignore the set operations
            % since they won't be set later.
            if ud.flags.calledby.dspblks 
                set(hfound{i}, 'Enable', this.Enable, frames(i).setops{:});
            else %not empty, and loading from matlab
                set(hfound{i}, 'Enable', this.Enable);
            end
        else %not empty and not loading
            set(hfound{i}, 'Enable', this.Enable, frames(i).setops{:});
        end
