...
hPanel = uipanel('Parent', hFig, ...
    ...
    'Position', pos, ...
    'Visible', 'Off', ...
    'Title', getString(message('signal:sigtools:siggui:CurrentFilterInfo')));

lbls = {getString(message('signal:sigtools:siggui:Structure')), getString(message('signal:sigtools:siggui:Order')), getString(message('signal:sigtools:siggui:Stable')), getString(message('signal:sigtools:siggui:Source'))};
...
% ----------------------------------------------------------------------
function update(this, eventData) %#ok<INUSD>
...
if isempty(Hd)
    stable = getString(message('signal:sigtools:siggui:Yes'));
    color  = [0 0 0];
    set(h.order,     'String', '50');
    set(h.structure, 'String', getString(message('signal:sigtools:siggui:DirectFormFIR')));
else
    ...
    if isstable(Hd)
        ...
        stable = getString(message('signal:sigtools:siggui:No'));
        color  = [1 0 0];
    end
end
...
if ~issamestructure
    ...
    hLayout.add(h.([tag '_lbl']), 1, 1, ...
        'TopInset',     15*sz.pixf, ...
        'Fill',         'Horizontal', ...
        'MinimumWidth', largestuiwidth(h.([tag '_lbl'])), ...
        'Anchor',       'South');
    ...
    set(h.(lower(labels{1})), 'String', getTranslatedString('signal:sigtools:siggui',fi.(labels{1})));
    ...
    for indx = 2:nlabels
        ...
        if ~isfield(h, tag)
            h.([tag '_lbl']) = uicontrol(hp, ...
                ...
                'Visible',             'Off', ...
                'ForegroundColor',     [0 0 1], ...
                'String',              sprintf('%s', getTranslatedString('signal:sigtools:siggui',interspace(labels{indx}))));
            h.(tag) = uicontrol(hp, ...
                'Style', 'Text', ...
                'HorizontalAlignment', 'Left', ...
                'Tag', tag, ...
                'Visible', 'Off');
            if strcmp(tag,'sections')
              set(h.(tag),'FontSize',get(h.('order'),'FontSize'));
            end
        end
    ...
    for indx = 1:length(labels)
        tag = lower(labels{indx});
        set(h.(tag), 'String',  getTranslatedString('signal:sigtools:siggui',fi.(labels{indx})));
    end
end
...
% -------------------------------------------------------------------------
function source_listener(this, eventData) %#ok<INUSD>

set(this.Handles.source, 'String', getTranslatedString('signal:sigtools:siggui',get(this, 'Source')));
