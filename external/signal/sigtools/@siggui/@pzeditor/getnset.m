...
if isempty(out)
    ...
    try
        roots = constructRoots(out);
        % Setting AllRoots triggers currentsection_listener so verify that
        % the current section setting is valid for the current filter        
        if this.CurrentSection > length(roots)
          this.CurrentSection = length(roots);
        end
        this.AllRoots = roots; 
        this.ErrorStatus = '';
        if isprop(this,'Handles')
          hdls = get(this,'Handles');        
          set(hdls.gain,'String', sprintf('%g',getgain(this)));
        end
    catch me
        this.ErrorStatus = getString(message('signal:sigtools:siggui:CouldNotCalculatePolesAndZeroes', ...
...
isNewFilterFir = signalpolyutils('isfir', num, den);

if isa(out, 'dfilt.dtfiir')
        set(out, 'Numerator', num, 'Denominator', den);
elseif isNewFilterFir && ...
        (isa(out,'dfilt.dtffir') || isa(out, 'mfilt.firinterp'))
      set(out, 'Numerator', num);  
elseif isNewFilterFir && isa(out,'mfilt.linearinterp') %holdinterp isa linearinterp
      %num is read only, need to create an firinterp instead
      out=mfilt.firinterp(out.InterpolationFactor,num);
else %will also convert to IIR if previous filter is a CIC or empty
...
if ~isempty(hC)
    ...
    if strcmpi(cmode, 'on')
        ...
        set(hObj, 'Roots', union(roots, h2,'legacy'));
    end
end
...
if sec < 1
    error(message('signal:siggui:pzeditor:getnset:InvalidSectionNotPositive'));
elseif sec > length(msroots)
    error(message('signal:siggui:pzeditor:getnset:InvalidSectionTooLarge'));
end
