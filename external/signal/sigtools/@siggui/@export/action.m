...
[varsExist, existMsgObj] = chkIfVarExistInWksp(tnames);
...
if ~overwriteVars & varsExist
    ...
    for i = 1:length(tnames)
        ...
        if isvarname(tnames{i})
            ...
            error(message('signal:siggui:Export1:action:InvalidName', tnames{ i }));
        end
    end
    
    % Message to be displayed in the command window. 
    exportMsg = getString(message('signal:sigtools:siggui:Variablesexportedworkspace'));
...
switch fileType
    case 'text'
        file = 'untitled.txt';
        dlgStr = getString(message('signal:sigtools:siggui:ExportCoefficientsTextfile'));
    case 'mat'
        file = 'untitled.mat';
        dlgStr = getString(message('signal:sigtools:siggui:ExportCoefficientsMATfile'));
end
...
if iscoeffs(hXP)
    ...
    error(message('signal:siggui:Export1:action:cannotExport'));
end
...
%-------------------------------------------------------------------
function [varsExist, existMsgObj] = chkIfVarExistInWksp(vnames)
...
%               MATLAB workspace.
%   existMsgObj  - Warning object containing dialog string to let the user 
%                  know that their
...
for n = 1:length(vnames)
    ...
    [vals, ~] = evaluatevars(vnames{n}); 
    
    % Put up a message if the variables exist
    if ~isempty(vals)
        varsExist = 1;
        existMsgObj = message('signal:siggui:Export1:action:VarExists',vnames{n});
