...
if isstruct(filtobj)
    ...
    if ~isfield(s, 'current_filt')
        error(message('signal:siggui:filtermanager:addfilter:InvalidParam'));
    end
    ...
    if ~strcmpi(name, 'force')
        newname = inputdlg({getString(message('signal:sigtools:siggui:EnterFilterNameToBeStoredInTheFilterManager'))}, ...
            getString(message('signal:sigtools:siggui:StoreFilter')), 1, {s.currentName});
        shouldcontinue = true;
        if isempty(newname)
            return;
        end
        ...
        while any(strcmpi(newname, allnames)) && shouldcontinue
            answer = questdlg(...
                getString(message('signal:sigtools:siggui:AFilterCalledAlreadyExistsDoYouWantToReplaceIt', newname{1})), ...
                getString(message('signal:sigtools:siggui:StoreFilter')), ...
                getString(message('signal:sigtools:siggui:Yes')), ...
                getString(message('signal:sigtools:siggui:No')), ...
                getString(message('signal:sigtools:siggui:Yes')));
            
            % If they say they want to replace, continue, otherwise ask for
            % more information.
            if strcmpi(answer, getString(message('signal:sigtools:siggui:Yes')))
                shouldcontinue = false;
                overwrite = true;
            else
                newname = inputdlg({getString(message('signal:sigtools:siggui:EnterFilterNameToBeStoredInTheFilterManager'))}, ...
                    getString(message('signal:sigtools:siggui:StoreFilter')), 1, newname);
    ...
    if isa(filtobj, 'dfilt.basefilter')
        ...
        error(message('signal:siggui:filtermanager:addfilter:InternalError'));
    end
end
