...
hm = addtargetmenu(hFDA,getString(message('signal:sigtools:sigtools:CodeComposerStudioIDE')),...
    cb,'export2ccs');
...
hEH = getcomponent(hFDA, 'tag', 'siggui.exportfilt2hw');
if isa(filtobj, 'dfilt.singleton') && ~isa(filtobj, 'dfilt.calattice') && ...
    ~isa(filtobj, 'dfilt.delay')
