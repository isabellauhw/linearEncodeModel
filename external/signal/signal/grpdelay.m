...
validateattributes(b,{'numeric'},{'nonempty'},'grpdelay','b');
validateattributes(a,{'numeric'},{'nonempty'},'grpdelay','a');
validateattributes(n,{'numeric'},{'nonempty'},'grpdelay','n');
validateattributes(Fs,{'numeric'},{'nonempty'},'grpdelay','fs');
...
if isreal(b) && isreal(a)
    ...
    numZerosInExcessOfPoles = max(0,length(z) - length(p));
    numPolesInExcessOfZeros = max(0,length(p) - length(z));
    
    % Remove zeros and poles at the origin.
    % Each zero at the origin in excess of the number of poles removes a unit delay
    k = find(abs(z) > toler);
    numZerosAtOrigin = length(z)-length(k);
    kNum = min(numZerosInExcessOfPoles, numZerosAtOrigin);
    gd = gd - T*(kNum);
    z = z(k);
    
    % Each pole at the origin in excess of number of zeros adds a unit delay
    k = find(abs(p) > toler);
    numPolesAtOrigin = length(p)-length(k);
    kNum = min(numPolesInExcessOfZeros, numPolesAtOrigin);
    gd = gd + T*(kNum);
    p = p(k);
