...
%
%   See also FINDSIGNAL, ALIGNSIGNALS, XCORR.
...
if ~isnumeric(x)
    % x must be numeric.
    error(message('signal:finddelay:firstInputNumeric'));
elseif ~ismatrix(x) || isempty(x)
    % x must be a vector or a matrix.
    error(message('signal:finddelay:firstInputVectorMatrix'));
end
...
if ~isnumeric(y)
    % y must be numeric.
    error(message('signal:finddelay:secondInputNumeric'));
elseif (~ismatrix(y)) || (isempty(y))
    % y must be a vector or a matrix.
    error(message('signal:finddelay:secondInputVectorMatrix'));
end
% fft function in xcorr only works on double and single data types.
y = double(y);    
if isvector(y)
...
if ~isvector(x) && ~isvector(y)
    if nx~=ny
        error(message('signal:finddelay:sameNumberColumns'));
    end    
end    
...
for i=1:length(d)
    if max_c(i)<1e-8
        ...
        elseif isvector(d) && maxlag(i)~=0
            warning(message('signal:finddelay:noSignificantCorrelationVector', i));
        end
    end    
end
