...
cellIdx = cellfun(@(x) iscell(x), varargin,'UniformOutput',true);
cellIdx = find(cellIdx == true);
[varargin{:}] = convertStringsToChars(varargin{:});
% Go into cell arrays inside varargin - we can have window specifications
% with parameters (e.g. {"kaiser",paramValue})
for idx = 1:numel(cellIdx)
    [varargin{cellIdx(idx)}{:}] = convertStringsToChars(varargin{cellIdx(idx)}{:});
end
...
if isempty(varargin)
  ...
  if outputVarNameParseFail
    ...
    % hdes is the handle to the filter design assistant design object
...
elseif isa(varargin{1},'digitalFilter')
  ...
  if isempty(inputParamValueName)
    ...
    % Call s, offer var name editor if we could not parse the
    % input variable name.
    D = varargin{1};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
  ...
  elseif isempty(err) && parseParams.ResponseOnly
    ...
    % Check preference and if we are coming from the command line
...
if isempty(varargin)
  ...
  if nargin > 1
    if isodd(length(varargin(2:end)))
...
for idx = 2:numel(inputParamNames)
  ...
  if ~isempty(parseParams)
    ...
    % errors out internally if it finds an ambiguity or an invalid
  ...
  if strcmpi(name,'Window') 
    ...
    elseif strncmp(currentParamValue,'{',1)
      ...
      if numel(kidx) == 2 && ~contains(currentParamValue,'@')
