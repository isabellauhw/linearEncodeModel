...
%  The 'lowpass', 'highpass', 'bandpass', and 'bandstop' functions are
%  implemented as local functions at the end of the cfirpm code. For the
%  other fucntions type PRIVATE/MULTIBAND, etc., for more information.
%
...
%     fvtool(b);
%
%  See also FIRPM, FIR1, FIRLS, FILTER, DESIGNFILT.
...
if nargin > 2
    filt_str = convertStringsToChars(filt_str);
end
...
% We need this code to find private functions or to bring lowpass,
% highpass, bandpass, and bandstop into the local scope (these are local
% functions)
fcns = functions(filt_call);
if isempty(fcns.file)...
        || isequal(fcns.function,'lowpass')...
        || isequal(fcns.function,'highpass')...
        || isequal(fcns.function,'bandpass')...
        || isequal(fcns.function,'bandstop')
...
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
% Predefined lowpass, highpass, bandpass, bandstop responses
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
...
if (Le == 4)
  if any(F < 0)
    error(message('signal:cfirpm:LowpassMustBePositive'));
  end
elseif (Le == 6)
  if F(3)*F(4) > 0
    error(message('signal:cfirpm:LowpassInvalidFreqVec'));
  end
else
  error(message('signal:cfirpm:LowpassInvalidDimensions'))
end
...
if (Le == 4)
  if any(F < 0)
    error(message('signal:cfirpm:HighpassMustBePositive')); 
  end
elseif (Le == 6)
  if F(3)*F(4) > 0
    error(message('signal:cfirpm:HighpassInvalidFreqVec'));
  end
else
  error(message('signal:cfirpm:HighpassInvalidDimensions'))
end
...
%  CFIRPM(N,F,@bandpass, ...) designs a linear-phase bandpass filter
%  response using CFIRPM.
%
%  CFIRPM(N,F,{@bandpass, D}, ...) specifies group-delay offset D such
%  that the filter response will have a group delay of N/2 + D in units of
%  the sample interval, where N is the filter order. Negative values create
%  less delay, while positive values create more delay.  By default, D=0.
%
...
%    b = cfirpm(30,[-1 -.5 -.4 .7 .8 1],@bandpass);
%    fvtool(b);  % View filter response.
%
%  EXAMPLE: Reduced group delay filter response.
%    b = cfirpm(30,[0 .6 .7 1],{@bandpass,-1});
%    fvtool(b);
