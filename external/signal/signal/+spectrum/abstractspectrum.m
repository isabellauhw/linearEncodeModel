classdef (CaseInsensitiveProperties=true) abstractspectrum < matlab.mixin.SetGet & matlab.mixin.Copyable
  ...
  methods
    ...
    function [x,nchans,msg] = checkinputsig(~,x)
      %CHECKINPUTSIG   Return the input vector column'ized & number of channels.
      %
      ...
      if xIsMatrix
        msg = getString(message('signal:spectrum:abstractspectrum:schema:MultichannelDatamatricesIsNotSupported'));
        return;
      else
        x = x(:);
        [~,nchans] = size(x);
    ...
    function fn = reorderEstimationMethod(~,fn)
    ...
    function thisloadobj(~, ~)
      %THISLOADOBJ   Load this object.
      
      
    end
    
    function varargout = thispsd(~,~,varargin)
      %THISPSD   Power Spectral Density (PSD) estimate.
