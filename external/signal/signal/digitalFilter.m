...
methods (Hidden)
  function obj = digitalFilter(varargin)
    ...
    % Cache filter type. Use numbers to speed up comparisons when asking
    % for filter type in other methods. 
    % 0 for FIR, 1 for IIR
    if isfir(obj.DfiltObject)
      obj.FilterType = 0;
    else
      obj.FilterType = 1;
    end
    ...
    if isempty(fdes) || fdes.NormalizedFrequency
      Fs = 2;
    else
      Fs = fdes.Fs;
    end
    ...
    dmIn = getdesignmethod(obj.DfiltObject);
    dm = [];
    if ~isempty(dmIn)
    ...
    if isempty(fdes) 
        if isfir(obj) && obj.DfiltObject.Numerator == 1
            fresp = 'allpass';
        else
            fresp = 'allstop';
        end
    ...
    pvalues = [pvalues; {dm; Fs; fresp; resp(end-2:end);...
      coeffPropValues}];
    
    for idx = 1:numel(pnames)
      if isempty(pvalues{idx})
          continue;
      end
      name = pnames{idx};
      obj.addprop(name); 
    ...
    % Setup filtering function handles depending of the filter type so that
    % we avoid if statements when filter methods are called. 
    if isfir(obj)
      obj.FilterFunction = @filterFIR;
      obj.FiltfiltFunction = @filtfiltFIR;
      obj.GetCoefficientsForAnalysisFunction = @getCoeffsForAnalysisFIR;
    else      
      obj.FilterFunction = @filterIIR;
      obj.FiltfiltFunction = @filtfiltIIR;      
      
      % Cache if the SOS matrix is single section so that we can add an
      % allpass section when performing analysis and filtering
      if size(obj.Coefficients,1) == 1
        obj.GetCoefficientsForAnalysisFunction = @getCoeffsForAnalysisIIRSOSSingleSection;        
      else
        obj.GetCoefficientsForAnalysisFunction = @getCoeffsForAnalysisIIR;
      end
...
methods  
  ...
  %
  %   [Z,P,K] = zplane(D) returns the zeros, poles, and gain of digital
  %   filter D.
  %
  ...
  s = '';
  if isempty(getfdesign(obj.DfiltObject))
      return;
  end
    try
  ...
  %   coefficients.
    ...
    % Cache metadata since it will be lost when we write to the df
    % coefficients
    fdes = getfdesign(df);
    fmthd = getfmethod(df);    
...
methods (Access = private)
  ...
  %getCoeffsForAnalysis Return a cell array with coefficients b,a or SOS
    ...
    c = obj.GetCoefficientsForAnalysisFunction(obj.Coefficients);
...
%------------------------------------------------------------------------
function y = filterFIR(c,x)
%filterFIR FIR filtering function
...
%------------------------------------------------------------------------
function y = filterIIR(c,x)
%filterIIR IIR filtering function
...
%------------------------------------------------------------------------
function y = filtfiltFIR(c,x)
%filtfiltFIR FIR filtfilt function
...
%------------------------------------------------------------------------
function y = filtfiltIIR(c,x)
%filtfiltIIR IIR filtfilt function

 y = filtfilt(c{:},1,x);
end

%------------------------------------------------------------------------
function c = getCoeffsForAnalysisFIR(coeff)
...
%------------------------------------------------------------------------
function c = getCoeffsForAnalysisIIR(coeff)
...
%------------------------------------------------------------------------
function c = getCoeffsForAnalysisIIRSOSSingleSection(coeff)
