    ...
    for I = 1:(info.nstages)
        ...
        for J = 1:length(Stg.nodeList)
            ...
            switch blk.blocktype
                case 'gain'
                    typeIdx(ndcount+J) = 1; %#ok<*AGROW>
                    prevNodeList(ndcount+J) = NaN;
                    if strcmp(DgDf.label,'statespace')
                        blk.label = blklabel;
                    elseif strncmp(fliplr(DgDf.label),'sos',3) && strcmp(Stg.mainParamList(J).params(I),'opsv')
                        try
                            prevIdx = blk.inport.from(1).node;
                            prevNodeList(ndcount+J) = prevIdx;
                        catch
                            prevNodeList(ndcount+J) = NaN;
                        end
                        typeIdx(ndcount+J) = 3;                        
...
prevNodeList = prevNodeList((typeIdx==3)|(typeIdx==5));

% If we have useless blocks, check that if the useless block is a gain the
% previous stage is not a cast stage. In this case, remove the gain block
% index as it will be deleted when the cast stage is deleted. 
tempblockslist = [];
for idx = 1:numel(uselessblockslist)    
    if isnan(prevNodeList(idx))
        tempblockslist = [tempblockslist uselessblockslist(idx)];               
    else
        if ~(strcmp(NList.nodes(uselessblockslist(idx)).block.blocktype,'gain') && strcmp(NList.nodes(prevNodeList(idx)).block.blocktype,'caststage'))
            tempblockslist = [tempblockslist uselessblockslist(idx)];               
        end
    end        
end
...
if doMapCoeffsToPorts
    ...
    for k = 1:length(uselessblockslist)
        ...
        if isempty(blktoberemoved.coeffnames)
            tempblockslist = [tempblockslist uselessblockslist(k)];
        end
    end
