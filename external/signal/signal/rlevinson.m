...
if any([signal.internal.sigcheckfloattype(a,'single','rlevinson','A(Prediction Polynomial)') ...
        signal.internal.sigcheckfloattype(efinal,'single','rlevinson','Efinal(Final Prediction Error)')])
...
a_temp = a_temp(:); % Convert to a column vector 

if a_temp(1)~=1
    coder.internal.warning('signal:rlevinson:InvalidParam');
    a_temp = a_temp/a_temp(1);
end

p = length(a_temp); % Prediction polynomial order p

if p < 2
    coder.internal.error('signal:rlevinson:InvalidDimensions');
end

% This matrix will have the prediction polynomials of orders 1:p
% Cast to enforce precision rules

U = complex(zeros(p,p,class(a_temp)));

U(:,p) = conj(a_temp(end:-1:1)); % Prediction coefficients of order p
p = p-1;

% First we find the prediction coefficients of smaller orders and form the
% Matrix U

e = complex(zeros(1,p,class(a_temp)));

% Initialize the step down
e(p) = efinal_temp;             % Prediction error of order p
temp_a = transpose(a_temp);
coder.varsize('temp_a','unbound');

% Step down
for k=p-1:-1:1
    [temp_a,e(k)] = levdown(temp_a,e(k + 1));
    U(:,k + 1) = [conj(temp_a(end:-1:1).');zeros(p - k,1)];
end

e0 = e(1)/(1-abs(temp_a(2)^2)); % Because a[1]=1 (true polynomial)
...
% Once we have the matrix U and the prediction error at various orders, we
% can use this information to find the autocorrelation coefficients.

R = complex(zeros(1,p,class(a_temp)));
...
% Actual recursion
for k = 2:1:p
    R(k) = -sum(conj(U(k-1:-1:1,k)).*R(k-1:-1:1).') - kr(k)*e(k - 1);
end
