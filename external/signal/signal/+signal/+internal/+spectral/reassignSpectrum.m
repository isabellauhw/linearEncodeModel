...
if coder.gpu.internal.isGpuEnabled
    outValRe = zeros(nf,nt, 'single');
    outValIm = zeros(nf,nt, 'single');
    for i = 1:length(rowIdx)
        if (rowIdx(i)>=1 && rowIdx(i)<=nf && colIdx(i)>=1 && colIdx(i)<=nt)
            outValRe(rowIdx(i),colIdx(i)) = ...
                gpucoder.internal.atomicAdd(outValRe(rowIdx(i),colIdx(i)), single(real(Sxx(i))));
            outValIm(rowIdx(i),colIdx(i)) = ...
                gpucoder.internal.atomicAdd(outValIm(rowIdx(i),colIdx(i)), single(imag(Sxx(i))));
        end
    end
    RSxx = cast(outValRe + 1i*outValIm, 'like', Sxx);
else
