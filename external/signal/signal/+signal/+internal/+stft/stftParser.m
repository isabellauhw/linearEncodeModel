function varargout = stftParser(fnName,x,varargin)
...
if ~isempty(varargin)
    ...
    if coder.target('MATLAB')
        ...
        pstruct.addParameter('FrequencyRange',[]);
        pstruct.addParameter('Centered',[]);
        if isInverse
        ...
        validateattributes(win,{'single','double'},...
            {'nonempty','finite','nonnan','vector','real'},fnName,'Window');
        win = win(:);
        nwin = length(win);
        ...
        freqRangeInit = pstruct.Results.FrequencyRange;
        
        if ~isempty(freqRangeInit)
            freqRange = freqRangeInit;
        else
        ...
        poptions = struct( ...
            ...
            'IgnoreNulls',true);
        if isInverse
            parms = {'Window','OverlapLength','FFTLength','Centered','Method', ...
                'InputTimeDimension','FrequencyRange'};
        else
            parms = {'Window','OverlapLength','FFTLength','Centered', ...
                'OutputTimeDimension','FrequencyRange'};
        end
        if ~isempty(idxParse)
            ...
            % Get centered value
            centered = coder.internal.getParameterValue(pstruct.Centered,[],varargin{1,idxParse});
            freqRangeInit = coder.internal.getParameterValue(pstruct.FrequencyRange,'',varargin{1,idxParse});
            
            % if frequency range is specified
            if ~isempty(freqRangeInit)
                freqRange = freqRangeInit;
            else
                freqRange = defaultFreqRange;
...
if ~isempty(freqRangeInit) && ~isempty(centered)
    coder.internal.error('signal:stft:FreqRangeError');
elseif ~isempty(centered)
...
freqRange = validatestring(freqRange,validStrings,fnName,'FrequencyRange');
...
[varargout{1},varargout{2}] = verifyDataAndTime(x,timeValue,pVals,fnName);

if nargout > 2
   varargout{3} =  timeDimension; % for codegen
end
...
if isInverse
    ...
    coder.internal.errorIf(~any(strcmpi(signalwavelet.internal.typeof(x),{'single','double'})),...
    ...
    % Verify class if not timetable in codegen
    coder.internal.errorIf(~coder.target('MATLAB')&&istimetable(x),'signal:stft:TimetableNotSupportedCodegen');
...
% Check if onesided
isOnesided = strcmp(pVals.FreqRange,'onesided');

if isOnesided
    if signalwavelet.internal.iseven(pVals.FFTLength)
        numFreqSamples = pVals.FFTLength/2+1;
    else
        numFreqSamples = (pVals.FFTLength+1)/2;
    end
else
   numFreqSamples =  pVals.FFTLength;
end
...
if (isInverse && strcmpi(pVals.TimeDimension,'acrosscolumns'))
    if ~isOnesided
        if pVals.WindowLength>size(data,1)
            coder.internal.error(sprintf('signal:%s:InvalidWindowLength',fnName), ...
                'rows',pVals.WindowLength);
        end
    else
        if numFreqSamples~=size(data,1)
            coder.internal.error(sprintf('signal:%s:InvalidFFTLength',fnName), ...
                'rows',numFreqSamples);
        end
    end
end

if (isInverse && strcmpi(pVals.TimeDimension,'downrows'))
    if ~isOnesided
        if pVals.WindowLength>size(data,2)
            coder.internal.error(sprintf('signal:%s:InvalidWindowLength',fnName), ...
                'columns',pVals.WindowLength);
        end
    else
        if numFreqSamples~=size(data,2)
            coder.internal.error(sprintf('signal:%s:InvalidFFTLength',fnName), ...
                'columns',numFreqSamples);
        end
    end
end

% Check if signal is complex when onesided
if ~isInverse
    coder.internal.errorIf(isOnesided&&~isreal(data),... 
        'signal:stft:InvalidInputDataTypeWithOnesided');
end
...
if any([isa(win,'single') isa(data,'single') ...
        (isa(data,'gpuArray') && strcmp(classUnderlying(data),'single'))])
    classCast = 'single';
    isSingle = true;
else
    classCast = 'double';
