...
%   be double or single but cannot be mixed. The output S contains a
%   two-sided and centered STFT for each signal channel. S is a matrix for
...
%
%   S = STFT(...,'FrequencyRange',FREQRANGE) returns a transform over the
%   specified range of frequencies based on the value of FREQRANGE:
%
%      'onesided' - returns a one-sided transform. Its frequency range is
%      over the range [0,pi] for even NFFT and [0,pi) for odd NFFT. When
%      time information is provided, the intervals become [0,Fs/2] and
%      [0,Fs/2), respectively.
%
%      'centered' - returns a two-sided and centered transform over the
%      frequency range (-pi,pi] for even NFFT and (-pi,pi) for odd NFFT.
%      When time information is provided, the intervals become (-Fs/2,Fs/2]
%      and (-Fs/2,Fs/2), respectively.
%
%      'twosided' - returns a two-sided transform over the range [0,2*pi).
%      When time information is provided, the interval becomes [0,Fs).
%
%      The default value of FREQRANGE is 'centered'. X cannot be complex
%      when FREQRANGE is 'onesided'.
%
...
%
%   STFT(...) with no output arguments plots the magnitude of the STFT.
%   This syntax does not support multichannel signals.
%
%    % EXAMPLE 1:
...
% Check inputs/outputs
    ...
    if coder.target('MATLAB')
        ...
        if ~allConst && coder.gpu.internal.isGpuEnabled
            [S,F,T] = computeSTFT_gpu(data,opts);
...
function [S,F,T] = computeSTFT_gpu(x,opts)
% Computes the short-time Fourier transform
    classCast = class(x);
    ...
    % Place x into columns and return the corresponding central time estimates
    [xin,t] = getSTFTColumns_gpu(x,nx,nwin,noverlap,Fs,win,nfft);
    
    % Apply the window to the array of offset signal segments and perform a DFT
    [S,f] = computeFFT(xin,nfft,Fs);
...
function [xin,t] = getSTFTColumns_gpu(x,nx,nwin,noverlap,Fs,win,nfft)
...
% Determine the number of columns of the STFT output (i.e., the S output)
    ...
    xin = coder.nullcopy(zeros(nfft,ncol,numChannels,'like', x));
    ...
    for iCh = 1:numChannels
        for icol = 1:ncol
            for ielem = 1:nfft
                if ielem <= nwin
                    xin(ielem, icol,iCh) = x((((icol-1)*nsamp) + ielem),iCh).*win(ielem);
                else
                    xin(ielem, icol,iCh) = 0;
                end
            end
        end
    end
    % Return time vector whose elements are centered in each segment
    t = ((0:(ncol-1))*(nwin-noverlap)+(nwin/2)')/Fs;
    % Magnitude (dB)
end
...
function [Xx,f] = computeFFT(xin,nfft,Fs)
    ...
    if coder.target('MATLAB')
        ...
        if isreal(xin)
            ...
            if isa(xin,'double')
                Xx = coder.nullcopy(zeros(size(xin),'like',1+1i));
                plan = gpucoder.internal.cufft.createcuFFTPlan(1, size(xin, 1), size(xin,1), 1, size(xin,1), ...
                                                               size(xin,1), 1, size(xin,1), 'CUFFT_D2Z', size(xin, 2)*size(xin, 3));
            else
                Xx = coder.nullcopy(zeros(size(xin),'like',single(1+1i)));
                plan = gpucoder.internal.cufft.createcuFFTPlan(1, size(xin, 1), size(xin,1), 1, size(xin,1), ...
                                                               size(xin,1), 1, size(xin,1), 'CUFFT_R2C', size(xin, 2)*size(xin, 3));
            end
            Xx = gpucoder.internal.cufft.executecuFFT(xin, Xx, plan, 'f');
            m = uint32(floor((size(xin,1)-1)/2));
            id = -mod(size(xin,1),2)+2;
            Xx(1+id+m:1+2*m,:,:) = conj(Xx(1+m:-1:2,:,:));
        else

            if isa(xin,'double')
                Xx = coder.nullcopy(zeros(size(xin),'like',1+1i));
                plan = gpucoder.internal.cufft.createcuFFTPlan(1, size(xin, 1), size(xin,1), 1, size(xin,1), ...
                                                               size(xin,1), 1, size(xin,1), 'CUFFT_Z2Z', size(xin, 2)*size(xin, 3));
            else
                Xx = coder.nullcopy(zeros(size(xin),'like',single(1+1i)));
                plan = gpucoder.internal.cufft.createcuFFTPlan(1, size(xin, 1), size(xin,1), 1, size(xin,1), ...
                                                               size(xin,1), 1, size(xin,1), 'CUFFT_C2C', size(xin, 2)*size(xin, 3));
            end
            Xx = gpucoder.internal.cufft.executecuFFT(xin, Xx, plan, 'f');
        end
    end

    f = coder.nullcopy(zeros(nfft,1));
