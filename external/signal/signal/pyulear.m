function varargout = pyulear(x,p,varargin)
%PYULEAR   Power Spectral Density (PSD) estimate via Yule-Walker's method.
%   Pxx = PYULEAR(X,ORDER) returns the PSD estimate, Pxx, of the
...
%   larger.
%
%   [Pxx,W] = PYULEAR(X,ORDER,NFFT) returns the vector of normalized
%   angular frequencies, W, at which the PSD is estimated.  W has units of
...
%
%   [Pxx,W] = PYULEAR(X,ORDER,W) computes the two-sided PSD at the 
%   normalized angular frequencies contained in vector W. W must have
%   at least two elements.
%
%   [Pxx,F] = PYULEAR(X,ORDER,NFFT,Fs) returns a PSD computed as a function
...
%
%   [Pxx,F] = PYULEAR(X,ORDER,F,Fs) computes the two-sided PSD at the 
%   frequencies contained in vector F.  F must contain at least two
%   elements and be expressed in hertz.
%
%   [...] = PYULEAR(...,FREQRANGE)  returns the PSD over the specified
...
%
%   [Pxx,F,Pxxc] = PYULEAR(...,'ConfidenceLevel',P) returns the P*100%
...
%   estimate Pxx(M,N).
%
%   PYULEAR(...) with no output arguments plots the PSD estimate (in decibels
%   per unit frequency) in the current figure window.
%
%   EXAMPLE:
%      x = randn(100,1);
%      y = filter(1,[1 1/2 1/3 1/4 1/5],x);
%      pyulear(y,4,[],1000);          % Uses the default NFFT of 256.
%
%   See also PBURG, PCOV, PMCOV, PMTM, PMUSIC, PWELCH, PERIODOGRAM, PEIG, 
%   ARYULE, PRONY.
...
% Precision rules for {x,p} and {NFFT,F,Fs}are enforced inside ARYULE and
