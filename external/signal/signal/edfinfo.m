...
methods (Access = private)
    ...
    function obj = readHeader(obj,filename,fid,fileInfo)
        ...
        if (annotationsIdxFlag)
            % Get annotations using readData where specifying signals and
            % records inputs as empty will only read annotations from the
            % file
            tAnnotations = signal.internal.edf.readData(fid,...
                filename, sigLabels, numDataRecords, physicalMaximum,...
                physicalMinimum, digitalMaximum, digitalMinimum,...
                numSignals, numSamples, [], [], dataRecordDuration, true);
        ...
        if ~issorted(tAnnotations)
            tAnnotations = sortrows(tAnnotations);
        end
        ...
        values = {string(fileInfo.name), string(datestr(fileInfo.datenum)),... 
            ...
            physicalMaximum, digitalMinimum, digitalMaximum, prefilter,...
            numSamples, sigReserve, tAnnotations};
