...
%   method.
%   Pxx = PMCOV(X,ORDER) returns the PSD estimate, Pxx, of the
...
%   larger.
%
%   [Pxx,W] = PMCOV(X,ORDER,NFFT) returns the vector of normalized angular 
...
%
%   [Pxx,W] = PMCOV(X,ORDER,W) computes the two-sided PSD at the normalized
%   angular frequencies contained in vector W. W must have at least two
%   elements.
%
%   [Pxx,F] = PMCOV(X,ORDER,NFFT,Fs) returns a PSD computed as a function of
...
%
%   [Pxx,F] = PMCOV(X,ORDER,F,Fs) computes the two-sided PSD at the 
%   frequencies contained in vector F. F must have at least two
...
%   estimate Pxx(M,N).
%
%   PMCOV(...) with no output arguments plots the PSD estimate (in decibels
