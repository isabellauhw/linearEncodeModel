...
[X,T] = computeISTFT_gpu(inputData,opts);
else
...
function [X,T] = computeISTFT_gpu(s,opts)
% Computes inverse short-time Fourier transform
classCast = class(s);
...
% Initialize time-domain signal
x = coder.nullcopy(zeros(xlen,numCh,'like', xifft));

normVal = coder.nullcopy(zeros(xlen,numCh,'like',win));
...
for dim3 = 1:numCh
    for dim2 = 1:nseg
        coder.gpu.kernel;
        for dim1 = (dim2-1)*hop+1:(dim2-1)*hop+nwin
            x(dim1,dim3) = x(dim1,dim3) ...
                + xifft(dim1 - (dim2-1)*hop,dim2,dim3)*(win(dim1 - (dim2-1)*hop))^a;
            normVal(dim1,dim3) = normVal(dim1,dim3)+(win(dim1 - (dim2-1)*hop))^(a+1);
        end
    end
end
...
if isa(y,'double')
    y2 = coder.nullcopy(zeros(nfft,size(y,2),size(y,3),'like',1+1i));
    y2(1:size(y,1),:,:) = y;
    x = coder.nullcopy(zeros(size(y2),'like',y2));
    plan = gpucoder.internal.cufft.createcuFFTPlan(1, size(y2, 1), size(y2,1), 1, size(y2,1), size(y2,1), 1, size(y2,1), 'CUFFT_Z2Z', size(y2,3)*size(y2, 2));
else
    y2 = coder.nullcopy(zeros(nfft,size(y,2),size(y,3),'like',single(1+1i)));
    y2(1:size(y,1),:,:) = y;
    x = coder.nullcopy(zeros(size(y2),'like',y2));
    plan = gpucoder.internal.cufft.createcuFFTPlan(1, size(y2, 1), size(y2,1), 1, size(y2,1), size(y2,1), 1, size(y2,1), 'CUFFT_C2C', size(y2,3)*size(y2, 2));
end
x = gpucoder.internal.cufft.executecuFFT(y2, x, plan, 'i');
