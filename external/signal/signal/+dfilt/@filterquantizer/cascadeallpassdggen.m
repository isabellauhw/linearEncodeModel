function DGDF = cascadeallpassdggen(q,Hd,coeffnames,doMapCoeffsToPorts,states)
% DGDF = cascadeallpassdggen(q, Hd) generate dgdf structure for 1st, 2nd
% and 4th order cascade allpass shared delay structure 
% Author : Honglei Chen
...
if isempty(coefs),
    ...
    DGDF = gen_DG_cascadeallpass_stages(coefs,q,info);
end
...
IC = local_getstates(hap,stagestates,info.states);

max_order = stagenum+1;

if max_order > 2
    Stg(1) = header(hap,stagestates,stagestartpt,info,IC.Stg1);
    for m = 2:max_order - 1
        stageic = IC.(sprintf('Stg%d',m));
        Stg(m) = body(hap,stagestates,stagestartpt,m,info,stageic);
    end
    stageic = IC.(sprintf('Stg%d',max_order));
    Stg(max_order) = footer(hap,stagestates,stagestartpt,info,stageic);
else
    Stg(1) = header(hap,stagestates,stagestartpt,info,IC.Stg1);
    Stg(2) = footer(hap,stagestates,stagestartpt,info,IC.Stg2);
end

if info.doMapCoeffsToPorts
    for stage = 1:stagenum
        lbl = info.coeffnames{stage};
        nstages = stagestates(stage);
        Stg(length(Stg)+1) = demux(q,hap,nstages,lbl);
    end
end

DGDF = filtgraph.dg_dfilt(Stg,'cascadeallpass');
...
%
% -------------------------------
function Head = header(hap,stagestates,stagestartpt,info,IC)

[NL, PrevIPorts, PrevOPorts, NextIPorts, NextOPorts, mainparams] = ElemBlockGen(hap,stagestates,stagestartpt,1,info,IC);
...
%
% --------------------------
function Body = body(hap,stagestates,stagestartpt,m,info,IC)

[NL, PrevIPorts, PrevOPorts, NextIPorts, NextOPorts, mainparams] = ElemBlockGen(hap,stagestates,stagestartpt,m,info,IC);
...
%
% ---------------------------
function Foot = footer(hap,stagestates,stagestartpt,info,IC)
...
%
% ---------------------------------
function [NL, PrevIPorts, PrevOPorts, NextIPorts, NextOPorts, mainparams] = ElemBlockGen(hap,stagestates,stagestartpt,stageidx,info,IC)
...
switch statenum
    case 1
        ...
        lbl = {};
        if info.doMapCoeffsToPorts
            lbl{1} = sprintf('%s%d',info.coeffnames{stageidx},1);
        end
        ...
        if stageidx > 1 && stagestates(stageidx-1)>stagestates(stageidx)
            for m = 1:stagestates(stageidx-1)-stagestates(stageidx)
                ...
                if mod(stageidx,2)
                    ...
                    mainparams(delayidx) = filtgraph.indexparam(delayidx,['1,' mat2str(IC(m+1,:))]);
                else
    ...
    case 2
        ...
        if info.doMapCoeffsToPorts
            lbl{1} = sprintf('%s%d',info.coeffnames{stageidx},1);
            lbl{2} = sprintf('%s%d',info.coeffnames{stageidx},2);
        end
        ...
        if stageidx > 1 && stagestates(stageidx-1)>stagestates(stageidx)
            for m = 1:stagestates(stageidx-1)-stagestates(stageidx)
                ...
                if mod(stageidx,2)
                    ...
                    mainparams(delayidx) = filtgraph.indexparam(delayidx,['1,' mat2str(IC(m+2,:))]);
                else
                    set(NL.nodes(delayidx).block,'orientation','left');
    ...
    case 4
        ...
        if info.doMapCoeffsToPorts
            for m = 1:4
                lbl{m} = sprintf('%s%d',info.coeffnames{stageidx},m);
            end
        end
...
%--------------------------------------------------------------------------
function IC = local_getstates(hap,stagestates,states)

max_order = length(hap)+1;
...
if max_order > 2
    ...
    for m = 2:max_order - 1
        ...
        % then the current stage has to render more delay blocks to
        % accommodate it.
        if stagestates(m-1)>stagestates(m)
            addedstates = stagestates(m-1)-stagestates(m);
            numstates = numstates + addedstates;
        end
