function DGDF = latticeardggen(q,Hd,coeffnames,doMapCoeffsToPorts,states)
...
DGDF = gen_DG_latticear_stages(q,num,Hd,info);
...
%
% -------------------------------------------------------------------------
function DGDF = gen_DG_latticear_stages(q,num,H,info,hTar)
...
elseif max_order == 1
    Stg = latticearheader_order0(q,num,H,info);
else
...
if info.doMapCoeffsToPorts && ~isempty(info.coeffnames)
    ...
    if max_order > 1
        % for first order latticear, K* is not used
...
DGDF = filtgraph.dg_dfilt(Stg,'latticear','rl');
...
set(NL.nodes(6),'position',[1.5 0 1.5 0]);
