function DGDF = latticemamaxdggen(q,Hd,coeffnames,doMapCoeffsToPorts,states)
...
DGDF = gen_DG_latticemamax_stages(q,num,Hd,info);
...
%
% -------------------------------------------------------------------------
function DGDF = gen_DG_latticemamax_stages(q,num,H,info,hTar)
...
elseif max_order == 1
    Stg = latticemamaxheader_order0(q,num,H,info);
else
...
if info.doMapCoeffsToPorts && ~isempty(info.coeffnames)
    ...
    % demux.
    Stg(length(Stg)+1) = demux(q,H,max_order,info.coeffnames{1});    % demux for K (K* if first order)
    if max_order > 1
        % for first order latticemamax, only K* is used, which is in 
        % coeffnames{1} in that particular case, 
...
DGDF = filtgraph.dg_dfilt(Stg,'latticemamax','lr');
...
set(NL.nodes(4),'position',[1.5 1 1.5 1]);
set(NL.nodes(4).block,'orientation','right');
mainparams(4)=filtgraph.indexparam(4,{});
...
mainparams(5)=filtgraph.indexparam(5,pgain,plabel);
