function DGDF = dffirtdggen(q,Hd,coeffnames,doMapCoeffsToPorts,states)
...
DGDF = gen_DG_dffirt_stages(q,num,Hd,info);
...
%
% -------------------------------------------------------------------------
function DGDF = gen_DG_dffirt_stages(q,num,H,info,hTar)
...
elseif max_order > 1
    ...
    Stg = dffirtheader_order0(q,num,H,info);
end

% Create demux.
if info.doMapCoeffsToPorts
    Stg(length(Stg)+1) = demux(q,H,max_order,info.coeffnames{1});    % demux 
end
...
DGDF = filtgraph.dg_dfilt(Stg,'dffirt');
...
set(NL.nodes(3).block,'label','HeadSumR');
...
NL.setnode(filtgraph.node('connector'),4);

set(NL.nodes(2).block,'label','BodyRSum');
set(NL.nodes(1).block,'label','b');
set(NL.nodes(3).block,'label','BodyRDelay');
...
set(NL.nodes(3),'position',[2 -0.8 2 -0.8]);  
set(NL.nodes(4),'position',[0.8 -0.2 0.8 -0.2]);
...
for stage = 2:(info.nstages-1)
    ...
    rsum_str{stage-1}='++|';  %left sum
    delay_str{stage-1}=['1,' mat2str(info.states(stage-1,:))];

end
mainparams(2) = filtgraph.indexparam(2,rsum_str);
mainparams(1) = filtgraph.indexparam(1,ng,lbl);
...
set(NL.nodes(1).block,'label','b');
set(NL.nodes(2).block,'label','BodyRDelay');
...
% point. x is positive towards right and y is positive towards bottom
set(NL.nodes(1),'position',[1 0 1 0]);  
set(NL.nodes(2),'position',[2 -0.8 2 -0.8]); 
...
mainparams(2) = filtgraph.indexparam(2,['1,' states]);
mainparams(1) = filtgraph.indexparam(1,ng,lbl);
