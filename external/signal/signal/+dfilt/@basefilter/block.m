...
if ~isempty(idx)
  ...
  ip = varargin{idx+1};
  if strcmpi(ip,'inherited')
      error(message('signal:dfilt:basefilter:block:InheritedDeprecated'));
  end
end
...
fldnames = fieldnames(s); % Used below for library block set_param

% In R2018b, Biquad Filter library block 'FilterSource' set_param value
% changed from 'dfilt object' to 'Filter object'. Handle that here.
if contains(Hd.FilterStructure,'Second-Order') && ...
        any(strcmpi(fldnames,'FilterSource'))
    logicalIndex = strcmpi(fldnames,'FilterSource');
    if isequal(s.(fldnames{logicalIndex}), 'dfilt object')
        s.(fldnames{logicalIndex}) = 'Filter object';
    end
end
