...
p = 0; msgObj = [];
N = length(y);
msgObj1 = message('signal:dfilt:dtfiir:limitcycle:NotEnoughData');
% Suppress fi overflow and underflow warnings on calling double
onStateOverFlow = warning('off','fixed:fi:overflow');
c1 = onCleanup(@() warning(onStateOverFlow));
onStateUnderFlow = warning('off','fixed:fi:underflow');
c2 = onCleanup(@() warning(onStateUnderFlow));
