...
if ~isnumeric(S)
    error(message('signal:dfilt:df1tsos:ziscalarexpand:MustBeNumeric'));
end
if issparse(S)
    error(message('signal:dfilt:df1tsos:ziscalarexpand:Sparse'));
end
...
if nsecs ~=0
    ...
    elseif strcmpi(class(S), 'filtstates.dfiir')
        error(message('signal:dfilt:df1tsos:ziscalarexpand:MustBeOneObject'));
    end
    ...
    if ~strcmpi(class(S), 'filtstates.dfiir')
        ...
        if size(Sinit,1) ~= statespersec
            error(message('signal:dfilt:df1tsos:ziscalarexpand:InvalidDimensions', statespersec));
        end
