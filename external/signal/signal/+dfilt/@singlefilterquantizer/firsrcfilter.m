...
if isreal(p)
    if isreal(x) && isreal(z)        
        [y,z,tapidx] = sfirsrcfilter(p,L,M,x,uint32(My),z,tapidx,im,inOffset);
    else        
...
zr = real(z);
[yr,zr,tapidx] = sfirsrcfilter(p,L,M,real(x),uint32(My),zr,tapidx,im,inOffset);

% Now filter imag part of input with imaginary initial conditions
zi = imag(z);
[yi,zi,tapidxc] = sfirsrcfilter(p,L,M,imag(x),uint32(My),zi,tapidxc,im,inOffset);
y = complex(yr,yi);
z = complex(zr,zi);
