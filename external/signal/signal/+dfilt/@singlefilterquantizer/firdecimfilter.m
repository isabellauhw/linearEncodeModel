...
if isreal(p)
  if isreal(x) && isreal(zi) && isreal(acc)
    % Call C-mex
    [y,zf,acc,phaseidx,tapidx] = sfirdecimfilter(p,M,x,zi,acc,phaseidx,tapidx,uint32(ny));
  else
    % Complex output
...
zir = real(zi);
accr = real(acc);
[yr,zir,accr,lclphaseidx,lcltapidx] = sfirdecimfilter(p,M,xr,zir,accr,lclphaseidx,lcltapidx,uint32(ny)); %#ok<NASGU,ASGLU>

% Imaginary part
xi = imag(x);
zii = imag(zi);
acci = imag(acc);
[yi,zii,acci,phaseidx,tapidx] = sfirdecimfilter(p,M,xi,zii,acci,phaseidx,tapidx,uint32(ny));
