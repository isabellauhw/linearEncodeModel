...
if isreal(p)
  if isreal(x) && isreal(zi) && isreal(acc)
    % Call C-mex
    [y,zf,acc,phaseidx] = sfirtdecimfilter(p,M,x,zi,acc,phaseidx,uint32(ny));
  else
    % Complex output
...
zir = real(zi);
accr = real(acc);
[yr,zir,accr,lclphaseidx] = sfirtdecimfilter(p,M,xr,zir,accr,lclphaseidx,uint32(ny)); %#ok<NASGU>

% Imaginary part
xi = imag(x);
zii = imag(zi);
acci = imag(acc);
[yi,zii,acci,phaseidx] = sfirtdecimfilter(p,M,xi,zii,acci,phaseidx,uint32(ny));
