...
if ~isfield(get(this),'Arithmetic')
    error(message('signal:dfilt:dffir:set2int:FixedPointLicense', 'SET2INT'));
end

if ~strcmpi(this.Arithmetic, 'fixed')
    error(message('signal:dfilt:dffir:set2int:FixedPointFcn', 'Arithmetic', 'fixed'));
end

if ~strcmpi(this.FilterInternals,'FullPrecision')
    error(message('signal:dfilt:dffir:set2int:FixedPointFcn', 'FilterInternals', 'FullPrecision'));
end
