...
if nargin > 3
    StopCriterion = convertStringsToChars(StopCriterion);
end

if ~isfdtbxinstalled
    error(message('signal:dfilt:abstractsos:limitcycle:DSPLicenseError', 'LIMITCYCLE'));
end

if ~isfixptinstalled
    error(message('signal:dfilt:abstractsos:limitcycle:FixPtLicenseError', 'LIMITCYCLE'));
end

if ~strcmpi(Hd.Arithmetic, 'fixed')
    error(message('signal:dfilt:abstractsos:limitcycle:ArithmeticError')); 
end
...
if ~isnumeric(Ntrials) || ~isscalar(Ntrials) || fix(Ntrials)~=Ntrials
    error(message('signal:dfilt:abstractsos:limitcycle:needScalar', 'Ntrials'));
end
...
if ~isnumeric(InputLengthFactor) || ~isscalar(InputLengthFactor) || fix(InputLengthFactor)~=InputLengthFactor
    error(message('signal:dfilt:abstractsos:limitcycle:needScalar', 'InputLengthFactor'));
end
if InputLengthFactor<2
    error(message('signal:dfilt:abstractsos:limitcycle:expectGreaterThan', 'InputLengthFactor'));
end
...
if isempty(strmatch(StopCriterion,{'either','granular','overflow'}))
    error(message('signal:dfilt:abstractsos:limitcycle:InvalidInput', 'StopCriterion'))
end

if ~isstable(Hd)
    warning(message('signal:dfilt:abstractsos:limitcycle:UnstableFilter'));
end
...
if (strcmpi(StopCriterion,'either') && strcmpi(LimitCycleType,'none')) || ...
    ...
    if strcmpi(warnstr, 'either')
        warning(message('signal:dfilt:abstractsos:limitcycle:EitherLimitCycleType', 'overflow', 'granular'));
    else
        warning(message('signal:dfilt:abstractsos:limitcycle:LimitCycleType', warnstr));        
    end
end
...
p = 0; msgObj = [];
N = length(y);
msgObj1 = message('signal:dfilt:abstractsos:limitcycle:NotEnoughData');
