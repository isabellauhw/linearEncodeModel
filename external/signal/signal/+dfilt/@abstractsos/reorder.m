...
numorder = convertStringsToChars(numorder);
...
if ischar(numorder)
    switch numorder
        ...
        otherwise
            ...
            newsv  = Hd.refScaleValues;
            oldsos = Hd.refsosMatrix;
            ...
            for indx = 1:size(newsos, 1)
                ...
                if length(newindx) == 1
                    ...
                    for jndx = 1:length(newindx)
                        if isempty(find(rindx == newindx(jndx), 1))
    ...
    if length(privsv) <= max(svorder)
        newsv = newsv(1:find(svorder <= length(privsv), 1, 'last' ));
    end
