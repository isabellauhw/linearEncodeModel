...
if ~isempty(S)
    ...
    if rem(size(S.Numerator,2), nsections)~=0
        error(message('signal:dfilt:df1sos:thissetstates:InvalidNumStateDimensions', nsections));
    end
    if rem(size(S.Denominator,2), nsections)~=0
        error(message('signal:dfilt:df1sos:thissetstates:InvalidDenomStateDimensions', nsections));
    end
