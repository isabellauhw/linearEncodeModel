function varargout = bandstop(x,varargin)
%BANDSTOP Filter signals with a bandstop filter
%   Y = BANDSTOP(X,Wpass) filters input X with a bandstop filter that has
%   passband frequencies defined by the two element row vector Wpass =
...
%   independently.
%
%   BANDSTOP performs zero phase filtering on input X using a bandstop
%   filter with a stopband attenuation of 60 dB. The bandstop filter
%   attenuates frequencies between the specified lower and upper passband
%   frequencies. BANDSTOP compensates for the delay introduced by the
%   filter, and returns output Y having the same dimensions as X.
%
%   Y = BANDSTOP(X,Fpass,Fs) specifies Fs as a positive numeric scalar
%   corresponding to the sample rate of X in units of hertz. Fpass =
%   [FpassLower, FpassUpper], is a two element row vector that defines the
%   stopband region of the filter in units of hertz.
%
%   YT = BANDSTOP(XT,Fpass) filters the data in timetable XT with a
%   bandstop filter that has passband frequencies defined by the two
%   element row vector, Fpass = [FpassLower, FpassUpper], in units of
%   hertz, and returns a timetable YT of the same size as XT. XT must
%   contain numeric double or single precision data. The row times must be
...
%
%   BANDSTOP(...,'Steepness',S) specifies the transition band steepness
...
%   bandstop response, but the resulting filter length and the
...
%
%   BANDSTOP(...,'StopbandAttenuation',A) specifies stopband attenuation,
%   A, of the filter as a positive scalar in dB. When not specified,
%   StopbandAttenuation defaults to 60 dB.
%
%   BANDSTOP(...,'ImpulseResponse',R) specifies the type of impulse
...
%
%   % EXAMPLE 2:
%      % Filter white noise with a bandstop filter with passband
%      % frequencies of [0.3*pi, 0.7*pi] rad/sample.
%      x = randn(1000,1);
%      bandstop(x,[0.3, 0.7])
%
%   % EXAMPLE 3:
%      % Filter white noise sampled at 1 kHz with a bandstop filter with
...
%      Fs = 1000;
%      x = randn(2000,1);
%      [y1, D1] = bandstop(x,[150,350],Fs,'Steepness',0.5);
%      [y2, D2] = bandstop(x,[150,350],Fs,'Steepness',0.8);
%      [y3, D3] = bandstop(x,[150,350],Fs,'Steepness',0.95);
...
%
%   See also LOWPASS, HIGHPASS, BANDPASS, FILTER, DESIGNFILT, DIGITALFILTER
...
opts = signal.internal.filteringfcns.parseAndValidateInputs(x,'bandstop',varargin);
...
if opts.WpassNormalized(2) >= 1
    ...
    % lowpass function.
    [y,D] = performLowpassFiltering(x,opts);
    opts.FilterObject = D;
    
    if opts.WpassNormalized(1) < 1
        % If all pass, let lowpass function  handle the warning
        warning(message('signal:internal:filteringfcns:ForcedLowpassDesign'));   
    end
else
...
%--------------------------------------------------------------------------
function [y,D] = performLowpassFiltering(x,opts)

if opts.IsTimetable
    [y,D] = lowpass(x,opts.Wpass(1),'Steepness',opts.Steepness(1),...
    ...
    if opts.IsNormalizedFreq
        ...
        [y,D] = lowpass(x,opts.Wpass(1),opts.Fs,'Steepness',opts.Steepness(1),...
            'StopbandAttenuation',opts.StopbandAttenuation);
    end
end
...
WcenterNormalized = WpassNormalized(1)/2 + WpassNormalized(2)/2;
if numel(opts.TwPercentage) == 1
    % Equal tw on both bands, chose min Tw
    TwPercentage = opts.TwPercentage;    
    Tw1 = TwPercentage * (WcenterNormalized - WpassNormalized(1));
    Tw2 = TwPercentage * (WpassNormalized(2) - WcenterNormalized);
    Tw = min(Tw1,Tw2);
    Tw1 = Tw;
    Tw2 = Tw;                   
else
    ...
    Tw1 = TwPercentage1 * (WcenterNormalized - WpassNormalized(1));
    Tw2 = TwPercentage2 * (WpassNormalized(2) - WcenterNormalized);    
end
...
F = [Wpass(1) Wstop(1)  Wstop(2) Wpass(2)];
A = [opts.PassbandRippleLinear opts.StopbandAttenuationLinear opts.PassbandRippleLinear];
NreqFir = kaiserord(F, [1 0 1], A, Fs);
...
if strcmp(impRespType,'iir')
    ...
    if opts.SignalLength <= 3*N
        ...
        params = {'bandstopiir', 'FilterOrder', N,...
        ...
        params = {'bandstopiir', 'PassbandFrequency1', Wpass(1),...
            'StopbandFrequency1', Wstop(1), 'StopbandFrequency2', Wstop(2),...
            'PassbandFrequency2', Wpass(2), 'PassbandRipple1', Apass,...
            'StopbandAttenuation', Astop, 'PassbandRipple2', Apass,...
            'DesignMethod', 'ellip'};        
    end
else
    % FIR design
    params = {'bandstopfir', 'PassbandFrequency1', Wpass(1),...
        'StopbandFrequency1', Wstop(1), 'StopbandFrequency2', Wstop(2),...
        'PassbandFrequency2', Wpass(2), 'PassbandRipple1', Apass,...
        'StopbandAttenuation', Astop, 'PassbandRipple2', Apass,...
...
aWpass = abs(sin(pi*WpassNormalized(2))/(cos(pi*WpassNormalized(2))-c));
ws1 = sin(pi*WstopNormalized(1))/(cos(pi*WstopNormalized(1))-c);
ws2 = sin(pi*WstopNormalized(2))/(cos(pi*WstopNormalized(2))-c);
