...
filterOrder = sps*span;
if abs((filterOrder) - round(filterOrder)) > sqrt(eps(class(filterOrder)))
  error(message('signal:rcosdesign:NonIntegerFilterOrder'))      
end
filterOrder = round(filterOrder);

if isodd(filterOrder)
  error(message('signal:rcosdesign:OddFilterOrder'))
end
