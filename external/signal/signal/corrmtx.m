...
%   X = CORRMTX(S,M) given a length N vector S and a positive integer M,
%   returns an N+M by M+1 matrix X such that X'*X is a (biased) estimate of
%   the M+1 by M+1 autocorrelation matrix of S. Note that N must be greater
%   than M.
%
...
validateattributes(M,{'double'},{'scalar','nonnegative','integer','<',N},'corrmtx','M');
