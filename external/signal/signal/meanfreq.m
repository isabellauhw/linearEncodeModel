...
coder.internal.assert(isempty(extraArgs),'signal:meanfreq:ExtraArgs');

% compute the mean frequency and power within the specified range
[f, pwr] = computeMeanFreq(Pxx, F, FrangeVal);

% plot if no output arguments specified (only in MATLAB execution)
if nargout==0
    coder.internal.assert(coder.target('MATLAB'),'signal:meanfreq:PlottingNotSupported');
    plotMeanFreq(Pxx, F, FrangeVal, rbw, f, status);
end
...
f = sum(bsxfun(@times,P(idx,:),F(idx)),1) ./ pwr;
