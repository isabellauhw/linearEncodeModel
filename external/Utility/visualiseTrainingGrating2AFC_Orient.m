function g = visualiseTrainingGrating2AFC_Orient(expRef, dataProfile, channelLabels)
    % This code visualises grating2afc photometry and behavioral data, used for current experiments.
    % Now based on EXPECTED REWARD VALUE of STIMULI
    ...
    %add a variable to split H and V stimuli
    Ori = abs(b.LeftOri - b.RightOri);
    Orilabels = cell(size(Ori));
    Orilabels(Ori == 0) = {'Vertical'};
    Orilabels(Ori == 90) = {'Horizontal'};
    Orilabels(Ori == 45) = {'45_Angle'};
    Orilabels(Ori == 135) = {'135_Angle'};
    %ExpectedValue = 
    %If need Mixed orientation label
    % Orilabels(Ori ~= 90 & Ori ~= 0) = {'Mixed/Angled'};

    expectedValue = unique(b.expectedRewardValue);
    expectedValue = sort(expectedValue, 'descend');
    ValueLabels = cell(size(b.expectedRewardValue));
    ValueLabels(b.expectedRewardValue == expectedValue(1)) = {sprintf('Large: %.1f', expectedValue(1))};
    ValueLabels(b.expectedRewardValue == expectedValue(2)) = {sprintf('Small: %.1f', expectedValue(2))};
    %{'Small', string(expectedValue(2))};
    ...
    largeValuePerformance = 100 * mean(b.feedback(goodTrials & b.expectedRewardValue == expectedValue(1)) == 'Rewarded');
    smallValuePerformance = 100 * mean(b.feedback(goodTrials & b.expectedRewardValue == expectedValue(2)) == 'Rewarded');
    ...
    % Combine data for vertical and horizontal trials
    %xData = [b.cDiff(vertTrials); b.cDiff(horizTrials)];
    %yData = [b.choice(vertTrials) == 'Right'; b.choice(horizTrials) == 'Right'];
    %groupData = [repmat({'Vertical Trials'}, sum(vertTrials), 1); repmat({'Horizontal Trials'}, sum(horizTrials), 1)];

    % Create the gramm object
    g(1, 1) = gramm('x', b.cDiff, 'y', b.choice=='Right', 'color', ValueLabels,'subset', goodTrials);

    % Plot the summary statistics
    g(1, 1).stat_summary('geom', {'point', 'line', 'errorbar'}, 'type', @grammCallbackBinomialConfidenceInterval);

    % Set properties and aesthetics
    g(1, 1).axe_property('ylim', [0 1]);
    g(1, 1).geom_vline('xintercept', 0, 'style', 'k:');
    g(1, 1).geom_hline('yintercept', 0.5, 'style', 'k:');
    g(1, 1).set_names('x', 'Contrast', 'y', 'p(Right) & 95% CI', 'color', 'Trial Type');
    g(1, 1).set_title(sprintf('%s\n%d good trials, %d total trials\nHighC: %0.2f%%, All: %0.2f%% correct,\nLrg: %0.2f%% ,Sml:%0.2f%%', expRef, sum(goodTrials), totalNumTrials, highCPerformance, overallPerformance,largeValuePerformance,smallValuePerformance));


    
    % Response time plot
    %Split for orientation: 
    g(2, 1) = gramm('x', b.cDiff, 'y', b.choiceCompleteTime - b.stimulusOnsetTime, 'color', ValueLabels, 'subset', b.feedback == 'Rewarded' & goodTrials);
    ...
    g(2, 1).set_title({'Response time' 'correct trials'});

    % Wheel position plot
    %yData_wheel = [wheel_warped(vertTrials,:); wheel_warped(horizTrials,:)];
    
    %g(3, 1) = gramm('x', 1:sum(warp_sizes), 'y', wheel_warped, 'color', b.choice, 'subset',b.feedback == 'Rewarded');
    g(3, 1) = gramm('x', 1:sum(warp_sizes), 'y', wheel_warped, 'color', ValueLabels,'subset',b.feedback == 'Rewarded', 'linestyle',b.choice );
    %g(3,1).facet_grid(Ori,[]);
    g(3, 1).stat_summary('setylim', 'true');
    g(3, 1).geom_vline('xintercept', cumsum(warp_sizes(1:3)), 'style', 'k:');
    g(3, 1).set_names('x', '', 'y', 'Wheel position mean & 95% CI','color', 'Orientation','linestyle', 'Choice');
    g(3, 1).set_title('Rewarded wheel movements');
    g(3, 1).axe_property('xtick', '', 'xcolor', 'none');

 
    % Right choices over time 
    ...
    %Rewarded choices over time 
    g(2,2) = gramm('x',b.trialNumber,'y', cumsum(b.choice == 'Right')./b.trialNumber,'subset',goodTrials);
    g(2,2) = gramm('x',b.trialNumber,'y', movmean(b.feedback == 'Rewarded', 10));
    g(2,2).geom_line();
    g(2,2).axe_property('ylim',[0 1]);
    g(2,2).geom_hline('yintercept',0.5,'style','k:');
    g(2,2).set_names('x','Trial number','y','p(Rewarded) (10 trial av.)');
    g(2,2).set_title('Rewarded choices over time') ;
    %g.draw();
    %{

    g(2, 2) = gramm('x', b.trialNumber, 'y', movmedian(b.choiceCompleteTime - b.stimulusOnsetTime, 20), 'color', b.choice, 'subset', goodTrials & horizTrials);
    g(2, 2).geom_line();
    g(2, 2).set_names('x', 'Trial number', 'y', 'Response time (good trials, 20 trial median.)', 'color', 'Choice');
    g(2, 2).set_title('Response time over time (good trials) - Horizontal OnLy');
      
    % Response time over time (good trials)
    g(3, 2) = gramm('x', b.trialNumber, 'y', movmedian(b.choiceCompleteTime - b.stimulusOnsetTime, 20), 'color', b.choice, 'subset', goodTrials & vertTrials);
    g(3, 2).geom_line();
    g(3, 2).set_names('x', 'Trial number', 'y', 'Response time (good trials, 20 trial median.)', 'color', 'Orientation');
    g(3, 2).set_title('Response time over time (good trials) - vertical OnLy');
    ...
    g(2, 2).set_title('Accuracy over time');
        ...
        % Accuracy over time - Orientation grouped
    g(3, 2) = gramm('x', b.trialNumber, 'y', movmean(b.feedback == 'Rewarded', 20), 'color', ValueLabels);
    g(3, 2).geom_line();
    g(3, 2).axe_property('ylim', [0 1]);
    g(3, 2).geom_hline('yintercept', 0.5, 'style', 'k:');
    g(3, 2).set_names('x', 'Trial number', 'y', 'Accuracy (20 trial av.)', 'color', 'Choice');
    g(3, 2).set_title('Accuracy over time - orientation');


%}
    % Check if channel labels are empty
    if isempty(channelLabels)
        ...
        figure('color', 'w', 'MenuBar', 'none', 'position', [364 457 1112 750]);
        g = g';
        g.draw();
        g.export('file_name', expRef,'export_path', '\\QNAP-AL001\JMoretti\Data\gramm','file_type','png');
    else
        % Get aligned data
